...
"""
分组增强版网络流量时间序列处理器
用于处理网络流量的时间序列数据，支持带分组信息的复杂数据格式处理
主要功能：
...
def write_2d_list_as_single_line(data, time_data, byte_data,group_list, max_num,  lable=None, filename=None):
    try:
        with open(filename, 'w') as file:
            ...
            row_str2 = " ".join(str(data) for data in group_list)
            file.write(row_str2 + '\t')
...
def  process_cell_file_no_t(file_path, lable, save_dir, max_pcap_num, column_num):
    ...
    try:
        with open(file_path, 'r') as file:
            ...
            write_2d_list_as_single_line(new_data_list_1, new_data_list_0,byte_list[:max_pcap_num], time_plot_list, max_pcap_num, lable, save_dir)
...
def train_tls():
    ...
    save_path = r"D:\处理的数据集\P-DTSF\p-tls1.3-50-5"
    if not os.path.exists(save_path):
        # 如果文件夹不存在，使用 os.makedirs() 创建它
        os.makedirs(save_path)
        print(f"文件夹 {save_path} 已创建")
    ...
    for p in t_list:
        ...
        s_dir = os.path.join(save_path, p)
        # 5, 30, 50, 100, 500
        process_cell_file_no_t(t_path, lable, s_dir,50,5)

        print("第 {} 个， 时间为 ：{}".format(num, time.time() - a))

    last_txt = r"D:\处理的数据集\P-DTSF\p-tls1.3-50-5.txt"
    s_txt = r"D:\处理的数据集\P-DTSF\p-tls1.3-50-5-s.txt"
...
def train_tor():
    ...
    save_path = r"D:\处理的数据集\TOR\tor-500-10"
    ...
    last_txt = r"D:\处理的数据集\TOR\tor-500-10.txt"
    s_txt = r"D:\处理的数据集\TOR\tor-500-10-s.txt"
...
def train_trojan():
    ...
    for p in t_list:
        ...
        s_dir = os.path.join(save_path, p)
        # 5, 30, 50, 100, 500
        process_cell_file_no_t_single(t_path, lable, s_dir,500,50)

        print("第 {} 个， 时间为 ：{}".format(num, time.time() - a))

    last_txt = r"D:\处理的数据集\trojan\trojan-single-500-50.txt"
    s_txt = r"D:\处理的数据集\trojan\trojan-single-500-50-s.txt"
