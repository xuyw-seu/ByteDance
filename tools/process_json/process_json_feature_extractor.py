...
# -*- coding: utf-8 -*-
"""
JSON Feature Extractor Tool

Core Functions:
- Process network traffic JSON files
- Extract time interval (iat) features
- Generate TSV/TXT files for model training
- Gather and organize TXT files
- Support complex filename parsing
    ...
    try:
        with open(file_path, 'r') as file:
            ...
            for k, v in data.items():
                ...
                time_list = packet_data_dict['iat']
                ...
                new_data_dict["mix_label"] = app_label
                new_name = str(file_index) + "-" + k + "-" + app_label + ".json"
...
def copy_files_to_different_folders(src_folder, dest_base_folder):
    ...
    for filename in os.listdir(src_folder):
        ...
        f_n = filename.split('-')[0] + "-" +filename.split('-')[1]
        # 检查文件是否为文件夹
        ...
        dest_folder = os.path.join(dest_base_folder, filename.split('-')[2].split('_')[0])
        # # 根据文件名的特定条件决定目标文件夹
...
def gather_txt_files(src_parent_folder, dest_folder):
    # 创建目标文件夹（如果不存在）
    if not os.path.exists(dest_folder):
        os.makedirs(dest_folder)

    # 遍历源父文件夹下的所有子目录
    for subdir, _, files in os.walk(src_parent_folder):
        # 遍历子目录下的所有文件
        for file in files:
            # 检查文件是否为 txt 文件
...
if __name__ == '__main__':
    s_path = r"D:\数据集\test_mobile_2"
    t_path = r"D:\数据集\mobile_app_2"
    s_list = os.listdir(s_path)
    a = time.time()
    index = 0
    for j in s_list:
        s_dir = os.path.join(s_path,j)
        t_dir = os.path.join(t_path,j)
    ...
    s_dir = r"D:\数据集\mobile app\22\Discord"
    t_dir = r"D:\数据集\mobile app\22-new\Discord"
    if not os.path.exists(t_dir):
        # 如果文件夹不存在，使用 os.makedirs() 创建它
        os.makedirs(t_dir)
        print(f"文件夹 {t_dir} 已创建")
    ...
    s_dir = r"D:\数据集\mobile app\22-8"
    t_dir = r"D:\数据集\mobile app\22-8"
    if not os.path.exists(t_dir):
        # 如果文件夹不存在，使用 os.makedirs() 创建它
        os.makedirs(t_dir)
        print(f"文件夹 {t_dir} 已创建")
    ...
    input_dir = r"D:\数据集\mobile app\22-cleaned\GotoMeeting-vc\GotoMeeting-vc-0.json"
    output_dir = r"C:\Users\Administrator\Desktop\test\1.txt"
    # os.makedirs(output_dir,exist_ok=True)
    data = input_dir + "%%" + output_dir + "%%" + "1"
    extract_2_feature_from_json(data)

    copy_files_to_different_folders(r"C:\Users\Administrator\Desktop\test\MIRAGE-2019_traffic_dataset_downloadable\Mi5_38_a4_ed_18_cc_bf",
                                    r"D:\数据集\test_mobile_2")
    gather_txt_files(r"D:\处理的数据集\mobile_app\mobile_eq_data", r"D:\处理的数据集\mobile_app\mobile_eq")